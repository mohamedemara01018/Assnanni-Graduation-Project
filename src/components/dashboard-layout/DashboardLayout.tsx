    ...
    <div>
      ...
      <div className="max-sm:relative">
        <div className="relative flex">
          ...
          <div
            className={`flex flex-col justify-between bg-(--color-surface) border border-(--color-border)  fixed bottom-0 top-0 left-0 h-screen  ${sidebarCollapsed ? `${collapsed ? "fixed top-0 bottom-0 left-0 z-20 w-60" : "hidden"}` : `${toggled ? "w-20" : "w-60"}`}`}
          >
            <SideBar
          ...
          <div
            className={`w-full ${sidebarCollapsed ? "" : `${toggled ? "ml-20" : "ml-60"}`} max-sm:m-auto `}
          >
            <div className="flex items-center justify-center h-16 border-b border-(--color-border) bg-(--color-surface) max-sm:sticky max-sm:top-0 z-20 w-full">
              <TobNavbar
                collapsed={sidebarCollapsed}
                setCollapsed={setCollapsed}
                pageTitle={pageTitle}
