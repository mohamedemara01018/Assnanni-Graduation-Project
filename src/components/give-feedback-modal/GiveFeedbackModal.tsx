...
import {
    ...
    Mail, ThumbsUp, Loader2,
} from "lucide-react";
import { useDispatch, useSelector } from "react-redux";
import { toast } from "react-toastify";
import type { AppDispatch } from "@/store/store";
import { addFeedbackState, fetchAddFeedback, resetAddFeedbackStatus } from "@/store/slices/patient-slice/add-feedback-slice/addFeedbackSlice";
...
function StarRow({
    rating, hover, onRate, onHover, onLeave, size = 7,
}: {
...
export function GiveFeedbackModal({
    ...
    const handleSubmit = () => {
        console.log(doctor.id, rating, comment);
                ...
                {showSuccess ? (
                    ...
                    <div className="px-8 py-12 text-center">
                        ...
                        <div className="mx-auto mb-5 inline-flex items-center gap-4 rounded-xl border px-6 py-3"
                            ...
                            <StarRow rating={rating} hover={0} onRate={() => { }} onHover={() => { }} onLeave={() => { }} size={4} />
                        ...
                        <div className="flex flex-col gap-5 px-7 py-5 overflow-y-auto flex-1 min-h-0">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-4">
                                    <StarRow rating={rating} hover={hover} onRate={setRating}
                                        onHover={setHover} onLeave={() => setHover(0)} size={8} />
                                    {(hover || rating) > 0 && (
                            ...
                            <label className="flex items-center gap-3 rounded-xl px-4 py-3 cursor-pointer"
                                style={{ backgroundColor: "var(--color-bg-blue)", border: "1px solid var(--color-primary-lighter)" }}>
                                <input type="checkbox" defaultChecked className="w-4 h-4 rounded accent-blue-600" />
                                <ThumbsUp className="w-4 h-4 shrink-0" style={{ color: "var(--color-primary)" }} />
