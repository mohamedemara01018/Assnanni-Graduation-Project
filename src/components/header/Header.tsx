    ...
    <header className="bg-(--color-surface) border-b-2 border-b-(--color-border) fixed top-0 left-0 right-0 z-20">
      <div className=" wrapper   flex items-center justify-between py-4 ">
        ...
        <div className="flex items-center gap-2">
          ...
          {isMobile && <button onClick={() => setMobileMenu(!showMobileMenu)} className='cursor-pointer p-2 rounded-xl hover:bg-(--color-bg-link-hover) transition-all duration-300 hover:scale-110'>
            {showMobileMenu ? <X className="w-5 h-5 text-foreground" /> : <List className="w-5 h-5 text-foreground" />}
          </button>}

          <ThemeToggle />

          {!isMobile && !user && <div className="flex items-center gap-2">
            <Link
              to={"/login"}
        ...
        {showMobileMenu && isMobile && <div className="fixed top-23 left-0 right-0 p-4 bg-(--color-surface) border border-t-2">
          <nav>
            ...
            {!user && <div className="flex flex-col pt-2 gap-2">
              <Link
                to={"/login"}
                className="text-(--color-primary) hover:bg-black/5 hover:dark:bg-white/15 py-2 px-4 rounded-sm cursor-pointer transform duration-200"
              >
