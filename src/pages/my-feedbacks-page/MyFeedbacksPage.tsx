...
import {
    fetchMyFeedbacks, myFeedbacksState, type FeedbackItem,
} from "@/store/slices/patient-slice/my-feedbacks-slice/myFeedbacksSlice";
import {
    deleteFeedbackState, fetchDeleteFeedback,
...
function Stars({ rating }: { rating: number }) {
    return (
        <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((s) => (
                <Star key={s} className={`w-3.5 h-3.5 ${s <= rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300 dark:text-gray-600"}`} />
            ))}
        </div>
    );
}
...
}: { feedback: FeedbackItem; onClose: () => void; onDeleted: () => void }) {
    const dispatch = useDispatch<AppDispatch>();
    const { loading } = useSelector(deleteFeedbackState);

    const handleDelete = async () => {
        try {
            await dispatch(fetchDeleteFeedback({ appointmentId: feedback.appointmentId })).unwrap();
                ...
                <div className="w-full max-w-sm rounded-2xl overflow-hidden"
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b"
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <p className="text-sm font-semibold" style={{ color: "var(--color-text)" }}>Delete Review</p>
                        </div>
                    ...
                    <div className="px-5 py-5">
                        <p className="text-sm leading-relaxed" style={{ color: "var(--color-text-light)" }}>
                            Your review for{" "}
                            <span className="font-semibold" style={{ color: "var(--color-text)" }}>{feedback.doctorName}</span>{" "}
...
function FeedbackCard({
    feedback, onEdit, onDelete,
}: { feedback: FeedbackItem; onEdit: (f: FeedbackItem) => void; onDelete: (f: FeedbackItem) => void }) {
    return (
        <div className="rounded-2xl border p-5 space-y-4 transition-all duration-150 hover:border-primary/30"
            ...
            <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3.5 min-w-0">
                    {feedback.doctorImage?.trim() ? (
                        <img src={feedback.doctorImage} alt={feedback.doctorName}
                            className="w-12 h-12 rounded-full object-cover shrink-0"
                            style={{ border: "1.5px solid var(--color-border)" }}
                            onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
                    ) : (
                        <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0"
                            style={{ backgroundColor: "var(--color-bg-blue)", border: "1.5px solid var(--color-border)" }}>
                            <User className="w-5 h-5" style={{ color: "var(--color-primary)" }} />
                        </div>
                    )}

                    <div className="min-w-0 space-y-1">
                        <Link to={`/doctors-list/${feedback.doctorId}`}
                        ...
                        <p className="text-xs flex items-center gap-1" style={{ color: "var(--color-text-light)" }}>
                            ...
                            {feedback.experienceYears > 0 && ` · ${feedback.experienceYears} yrs`}
                        </p>
                        {feedback.clinicName && (
                            <p className="text-xs flex items-center gap-1" style={{ color: "var(--color-text-light)" }}>
                                <Building2 className="w-3 h-3 shrink-0" />
                                {feedback.clinicName}{feedback.clinicLocation && ` · ${feedback.clinicLocation}`}
                            </p>
                        )}
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                    <div className="flex items-center gap-1 text-[11px]" style={{ color: "var(--color-text-light)" }}>
                        <Calendar className="w-3 h-3" />
                        {formatDate(feedback.createdAt)}
                    </div>
                    <div className="flex items-center gap-1.5">
                        <button onClick={() => onEdit(feedback)} title="Edit review"
                        ...
                        <button onClick={() => onDelete(feedback)} title="Delete review"
                            className="w-7 h-7 rounded-lg flex items-center justify-center transition-opacity hover:opacity-70 cursor-pointer"
            ...
            <div className="flex items-center justify-between pt-1">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs"
                    style={{ backgroundColor: "rgba(22,163,74,0.08)", color: "var(--color-success)", border: "1px solid rgba(22,163,74,0.15)" }}>
                    <ThumbsUp className="w-3 h-3" />
                    Would recommend
                </div>
                <Link to={`/appointments/booking/${feedback.doctorId}`}
...
export default function MyFeedbackPage() {
    ...
    useEffect(() => { refetch(); }, [dispatch, search, pageNumber, pageSize, ratingFilter]);

    const handleSearch = (v: string) => { setSearch(v); setPageNumber(1); };
    const handleRating = (v: number | null) => { setRatingFilter(v); setPageNumber(1); };

    const items = data?.items ?? [];
    const averageRating = data.totalCount > 0 && items.length > 0
        ...
        <DashboardLayout pageTitle="My Feedback">
            <div className="space-y-5">
                ...
                <div className="rounded-2xl border p-6"
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h1 className="text-2xl font-semibold mb-0.5" style={{ color: "var(--color-text)" }}>
                                My Feedback & Reviews
                            </h1>
                        ...
                        <div className="flex items-center gap-8">
                            <div className="text-center">
                                <p className="text-3xl font-bold" style={{ color: "var(--color-primary)" }}>{data.totalCount}</p>
                                <p className="text-xs mt-0.5" style={{ color: "var(--color-text-light)" }}>Total Reviews</p>
                            </div>
                            <div className="w-px h-10" style={{ backgroundColor: "var(--color-border)" }} />
                            <div className="text-center">
                                <p className="text-3xl font-bold" style={{ color: "#ca8a04" }}>{averageRating}</p>
                                <p className="text-xs mt-0.5" style={{ color: "var(--color-text-light)" }}>Avg. Rating</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                        {[null, 5, 4, 3, 2, 1].map((r) => (
                            <button key={r ?? "all"}
                                ...
                                {r === null ? "All" : <><Star className="w-3 h-3 fill-current" />{r}</>}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                {/* ── Content ──────────────────────────────────────────────── */}
                {loading ? (
                    <div className="flex justify-center py-12"><MiniLoading /></div>
                ) : error ? (
                    <Error message={error} />
                ) : items.length === 0 ? (
                    <NotFound
                        ...
                        onReset={search || ratingFilter ? () => { handleSearch(""); handleRating(null); } : undefined}
                        ...
                        <div className="space-y-4">
                            {items.map((feedback) => (
                                <FeedbackCard key={feedback.feedbackId} feedback={feedback}
                                    onEdit={setEditingFeedback} onDelete={setDeletingFeedback} />
                            ))}
                        </div>
                        <Pagination
                            pageNumber={pageNumber} pageSize={pageSize} totalItems={data.totalCount}
            ...
            {editingFeedback && (
                <FeedbackModal
                    ...
                    onSaved={() => { setEditingFeedback(null); refetch(); }}
                    mode="edit"
            ...
            {deletingFeedback && (
                <DeleteConfirmModal
                    ...
                    onDeleted={() => { setDeletingFeedback(null); refetch(); }}
                />
            )}
        </DashboardLayout>
    );
