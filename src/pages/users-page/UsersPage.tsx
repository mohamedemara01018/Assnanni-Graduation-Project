...
function getRoleStyle(role: string) {
  return roleConfig[role.toLowerCase()] ?? {
    bg: "var(--color-bg)",
...
function StatCard({ label, value, color }: { label: string; value: number | undefined; color?: string }) {
  return (
    ...
    >
      <p className="text-xs font-medium" style={{ color: "var(--color-text-light)" }}>{label}</p>
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: user.isActive ? "var(--color-success)" : "#dc2626" }}
        />
        ...
        <div className="px-6 py-5 space-y-3">
          ...
          <div
            ...
            style={{
              background: user.isActive ? "rgba(22,163,74,0.07)" : "rgba(220,38,38,0.06)",
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold" style={{ color: user.isActive ? "var(--color-success)" : "#dc2626" }}>
          ...
          {[
            ...
            { icon: <Phone className="w-3.5 h-3.5" />, label: "Phone", value: user.phoneNumber || "—" },
            { icon: <User className="w-3.5 h-3.5" />, label: "Gender", value: user.gender || "—" },
            { icon: <Calendar className="w-3.5 h-3.5" />, label: "Joined", value: new Date(user.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" }) },
          ].map((row) => (
            <div
              key={row.label}
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-[10px] uppercase tracking-widest font-semibold" style={{ color: "var(--color-text-light)" }}>
                  {row.label}
                </p>
...
function UsersPage() {
  ...
  const { usersData, totalCount, loading: usersLoading } = useSelector(selectUsers) as UsersState;
  ...
  const stats = [
    ...
    { label: "Receptionists", value: data?.totalReceptionists, color: undefined },
    { label: "Active", value: data?.totalActionedToday, color: "var(--color-success)" },
        ...
        >
          <div className="flex flex-col lg:flex-row gap-3">
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              {[
                {
                  onChange: (e: React.ChangeEvent<HTMLSelectElement>) => setFilterRole(e.target.value as UserRole),
                  options: [
                ...
                {
                  onChange: (e: React.ChangeEvent<HTMLSelectElement>) => setFilterGender(e.target.value as UserGender),
                  options: [
        ...
        >
          {usersLoading ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-xs animate-pulse" style={{ color: "var(--color-text-light)" }}>
                Loading users…
              </p>
            </div>
          ) : usersData.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <AlertCircle className="w-10 h-10" style={{ color: "var(--color-border)" }} />
              <p className="text-sm" style={{ color: "var(--color-text-light)" }}>No users found</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ borderBottom: "1px solid var(--color-border)", background: "var(--color-bg)" }}>
                    {["User", "Contact", "Role", "Gender", "Status", "Joined", "Actions"].map((h) => (
                      <th
                        key={h}
                  ...
                  {usersData.map((user, idx) => {
                    ...
                    return (
                      <tr
                        ...
                        style={{
                          borderBottom: idx < usersData.length - 1 ? "1px solid var(--color-border)" : "none",
                        }}
                      ...
                      >
                        ...
                        {/* Gender */}
                        <td className="px-5 py-4">
                          <span className="text-xs capitalize" style={{ color: "var(--color-text-light)" }}>
                            {user.gender || "—"}
                          </span>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <span
                            className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full font-semibold"
                            style={{
                              background: user.isActive ? "rgba(22,163,74,0.08)" : "rgba(220,38,38,0.08)",
