...
import {
  ...
  selectAIModelsState,
  type AIModelItem,
} from "@/store/slices/admin-slice/ai-model-slice/aiModelsSlice";
import {
  deleteAIModel,
  selectDeleteAIModelState,
} from "@/store/slices/admin-slice/delete-aI-model-slice/deleteAIModelSlice";
...
function ModelCard({
  ...
  return (
    ...
    >
      ...
      {/* Metric bars */}
      <div
        className="mt-4 pt-4 grid grid-cols-2 gap-4 border-t"
        style={{ borderColor: "var(--color-border)" }}
      >
        {[
...
export default function AIModelHistoryPage() {
  ...
  const handleDelete = async () => {
    ...
    const successMessage = await dispatch(
      deleteAIModel(selectedModel.id),
    ).unwrap();
    dispatch(fetchAllAIModels());
  ...
  return (
    <DashboardLayout pageTitle="AI Models">
      <div className="space-y-5">
        ...
        {loading ? (
          <div className="py-10">
            <MiniLoading message="Loading AI models…" />
          </div>
        ) : error ? (
          <Error message={error} />
        ) : models.length === 0 ? (
          ...
          >
            ...
            <p className="text-xs" style={{ color: "var(--color-text-light)" }}>
              Deployed models will appear here
            </p>
          </div>
        ) : (
