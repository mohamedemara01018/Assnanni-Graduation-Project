...
function InfoRow({
    ...
    return (
        <div
            className="flex flex-col gap-1 p-3.5 rounded-xl bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800 animate-in fade-in duration-150"
        >
    ...
    return (
        <>
            ...
            <div className="fixed inset-0 z-45 flex items-center justify-center p-4">
                <div
                    className="relative w-full max-w-3xl flex flex-col rounded-2xl bg-white dark:bg-gray-900 shadow-2xl border border-gray-100 dark:border-gray-800 max-h-[90vh] overflow-hidden"
                >
                    {/* Modal header */}
                    <div
                        className="flex items-center justify-between px-7 py-5 border-b border-gray-100 dark:border-gray-800 shrink-0"
                    >
                        <div className="flex items-center gap-3.5">
                            ...
                            >
                                {doctor.fullProfileImageUrl ? (
                                    <img src={doctor.fullProfileImageUrl} alt={doctor.fullName} className="w-full h-full object-cover" />
                                ) : (
                                    doctorInitials
                                )}
                            </div>
                    ...
                    <div className="p-7 space-y-6 overflow-y-auto flex-1 min-h-0">
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <InfoRow icon={<Phone className="w-3.5 h-3.5" />} label="Phone" value={doctor.phoneNumber} />
                                <InfoRow icon={<Phone className="w-3.5 h-3.5" />} label="Clinic Phone" value={doctor.clinicPhoneNumber} />
                                <InfoRow icon={<User className="w-3.5 h-3.5" />} label="Gender" value={doctor.gender} />
                                <InfoRow icon={<Calendar className="w-3.5 h-3.5" />} label="Birth Date" value={doctor.birthDate} />
                                <InfoRow icon={<MapPin className="w-3.5 h-3.5" />} label="Country" value={doctor.country} />
                                <InfoRow icon={<MapPin className="w-3.5 h-3.5" />} label="City" value={doctor.city} />
                                <InfoRow icon={<MapPin className="w-3.5 h-3.5" />} label="Street" value={doctor.street} />
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <InfoRow icon={<Hash className="w-3.5 h-3.5" />} label="License Number" value={doctor.medicalLicenseNumber} />
                                <InfoRow icon={<Briefcase className="w-3.5 h-3.5" />} label="Experience" value={doctor.yearsOfExperience ? `${doctor.yearsOfExperience} years` : null} />
                                <InfoRow icon={<DollarSign className="w-3.5 h-3.5" />} label="Consultation Fee" value={doctor.price ? `$${doctor.price}` : null} />
                                <InfoRow icon={<Award className="w-3.5 h-3.5" />} label="Degree" value={doctor.degree} />
                                <InfoRow icon={<GraduationCap className="w-3.5 h-3.5" />} label="Education" value={doctor.education} />
                                <InfoRow icon={<Languages className="w-3.5 h-3.5" />} label="Languages" value={doctor.languages} />
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <InfoRow icon={<Building2 className="w-3.5 h-3.5" />} label="Clinic Name" value={doctor.clinicName} />
                                <InfoRow icon={<MapPin className="w-3.5 h-3.5" />} label="Clinic Location" value={doctor.clinicLocation} />
                            </div>
                        </section>
                        ...
                        {doctor.about && (
                            <section>
                                ...
                                <div
                                    className="p-4 rounded-xl text-sm leading-relaxed bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800 text-gray-900 dark:text-white"
                                >
                                    {doctor.about}
                                </div>
                            </section>
                        ...
                        {doctor.fullCertificateUrl && (
                            <section>
                                ...
                                <div
                                    className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl gap-4 bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-800"
                                >
                                    <div className="flex items-center gap-3">
                                        <div
                                            className="w-10 h-10 rounded-lg flex items-center justify-center bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 shrink-0"
                                        >
                    ...
                    {/* Actions */}
                    <div
                        className="flex flex-col sm:flex-row gap-3 px-7 py-4 border-t border-gray-100 dark:border-gray-800 shrink-0 bg-gray-50/50 dark:bg-gray-900/50 rounded-b-2xl"
                    >
                        <button
                            onClick={onReject}
...
export default function VerifyDoctorsPage() {
    ...
    const [targetDoctor, setTargetDoctor] = useState<SearchPendingDoctor | null>(null);
    ...
    useEffect(() => {
        dispatch(fetchSearchPendingDoctor({ searchTerm: searchQuery, pageNumber: 1, pageSize: 10 }));
    ...
    const handleConfirmApprove = async (doctorId: string, note: string) => {
        ...
        try {
            const res = await dispatch(fetchApprovePendingDoctor({ note, id: doctorId })).unwrap();
    ...
    const handleConfirmReject = async (doctorId: string, reason: string) => {
        setSelectButton(doctorId);
        try {
            const res = await dispatch(fetchRejectPendingDoctor({ reason, id: doctorId })).unwrap();
        ...
        <DashboardLayout pageTitle="Verify Doctors">
            ...
            {loading ? (
                ...
                >
                    ...
                    <p className="text-sm" style={{ color: "var(--color-text-light)" }}>No verifications found</p>
                </div>
            ) : (
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        <button
                                            ...
                                            onClick={() => { setTargetDoctor(doctor); setShowApproveModal(true); }}
                                        ...
                                        <button
                                            disabled={isThisLoading}
                                            onClick={() => { setTargetDoctor(doctor); setShowRejectModal(true); }}
                                            className="p-2 rounded-lg flex items-center gap-1.5 text-xs font-medium text-white transition-opacity hover:opacity-85 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-[0.98] transition-transform duration-100"
                                            style={{ backgroundColor: "#dc2626" }}
                                        >
                                ...
                                <div className="flex items-center gap-1.5 flex-wrap">
                                    {doctor.email && <InfoPill icon={<Mail className="w-3 h-3" />} label={doctor.email} />}
                                    {doctor.phoneNumber && <InfoPill icon={<Phone className="w-3 h-3" />} label={doctor.phoneNumber} />}
                                    {doctor.yearsOfExperience > 0 && <InfoPill icon={<Briefcase className="w-3 h-3" />} label={`${doctor.yearsOfExperience} yrs exp`} />}
                                    {doctor.price > 0 && <InfoPill icon={<DollarSign className="w-3 h-3" />} label={`$${doctor.price} / visit`} />}
                                    {doctor.education && <InfoPill icon={<GraduationCap className="w-3 h-3" />} label={doctor.education} />}
                                    {doctor.languages && <InfoPill icon={<Languages className="w-3 h-3" />} label={doctor.languages} />}
                                    {doctor.country && <InfoPill icon={<Globe className="w-3 h-3" />} label={`${doctor.city ? doctor.city + ", " : ""}${doctor.country}`} />}
                                    {doctor.medicalLicenseNumber && <InfoPill icon={<Hash className="w-3 h-3" />} label={doctor.medicalLicenseNumber} />}
                                </div>
            ...
            {showApproveModal && targetDoctor && (
                <ApproveDoctorModal
                    ...
                    doctor={targetDoctor}
                    submitting={selectButton === targetDoctor.doctorId && approvedDoctor.loading}
                />
            )}
            ...
            {showRejectModal && targetDoctor && (
                <RejectDoctorModal
                    ...
                    doctor={targetDoctor}
                    submitting={selectButton === targetDoctor.doctorId && rejectedDoctor.loading}
                />
            )}
        </DashboardLayout>
    );
                ...
                <div className="flex flex-col gap-5 px-7 py-5 overflow-y-auto flex-1 min-h-0">
                    ...
                    <div className="flex items-start gap-3 rounded-xl bg-gray-50 dark:bg-gray-800/60 px-4 py-3.5 border border-gray-100 dark:border-gray-800">
                        ...
                        <div>
                            ...
                            <h4 className="text-sm font-medium text-gray-900 dark:text-white">{doctor.fullName}</h4>
                    ...
                    <div className="flex items-start gap-2.5 rounded-xl bg-red-50 dark:bg-red-900/20 px-4 py-3 border border-red-100/30 dark:border-red-900/30">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-red-600 dark:text-red-400 font-light leading-relaxed">
                                You are about to reject this application. This action is permanent, and the doctor will be notified of the reason.
                            </p>
                        </div>
                    </div>
