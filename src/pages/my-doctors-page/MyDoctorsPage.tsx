  ...
  return (
    <DashboardLayout pageTitle="My Doctors">
      <div className="space-y-5">
        ...
        {loading ? (
          <div className="py-10">
            <MiniLoading message="Loading your doctors…" />
          </div>
        ) : error ? (
          <Error message={error} />
        ) : data.items?.length === 0 ? (
          ...
          <NotFound
            message={
              search ? "No reviews match your filters" : "No feedback yet"
            }
            subMessage={
              search
      ...
      {selectedDoctor && (
        <FeedbackModal
          ...
          mode={"add"}
          appointmentId={Number(selectedDoctor.appointmentId)}
        />
      )}
    </DashboardLayout>
  );
  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex items-start gap-4">
        ...
        <div className="relative shrink-0">
          {doctor.imageUrl ? (
            ...
            <div className="w-16 h-16 rounded-xl bg-(--color-bg-blue) border border-primary/20 flex items-center justify-center">
      ...
      <div className="mx-5 mb-4 grid grid-cols-2 gap-2 rounded-xl bg-(--color-bg) border border-border p-3">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-(--color-text-light) mb-0.5">
            Last Visit
          </p>
          <p className="text-xs font-medium text-(--color-text)">
            {formatDateTime(doctor.lastAppointmentDate).date}
          </p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-widest text-(--color-text-light) mb-0.5">
          ...
          <p className="text-xs font-medium text-(--color-text)">
            {doctor.prescriptions?.length ?? 0} record
            {doctor.prescriptions?.length !== 1 ? "s" : ""}
          </p>
        </div>
      </div>
      ...
      {lastFeedback?.comment && (
        <div className="mx-5 mb-4 flex items-start gap-2 rounded-xl bg-(--color-bg-blue) border border-primary/20 px-3 py-2.5">
      ...
      {hasPrescriptions && (
        <div className="mx-5 mb-4 rounded-xl border border-border bg-(--color-surface) divide-y divide-border overflow-hidden">
          <div className="px-3 py-2 flex items-center justify-between">
            <p className="text-[10px] uppercase tracking-widest font-medium text-(--color-text-light)">
              Latest Prescription
            </p>
            <Link
              to={`/prescriptions`}
            ...
            <div key={rx.id} className="px-3 py-2 space-y-1">
              ...
              <div className="flex flex-wrap gap-1">
                {rx.items.slice(0, 3).map((item, i) => (
                  <span
                    key={i}
                    className="text-[10px] bg-(--color-bg) border border-border text-(--color-text-light) rounded-md px-1.5 py-0.5"
                  >
