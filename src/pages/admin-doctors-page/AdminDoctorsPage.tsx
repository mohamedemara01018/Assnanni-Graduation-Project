        ...
        >
          ...
          <div>
            ...
            <p
              ...
              style={{ color: "var(--color-text-light)" }}
            >
              You are about to reject this doctor. This action will deactivate
              their profile and restrict login. A rejection reason is required.
            </p>
          </div>
        </div>
      ...
      {!isVerified && (doctor as RejectedDoctor).rejectionReason && (
        ...
        >
          ...
          <span className="truncate">
            Reason: {(doctor as RejectedDoctor).rejectionReason}
          </span>
        </div>
      )}
...
export default function AdminDoctorsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"verified" | "rejected">(
    "verified",
  );
  const [searchTerm, setSearchTerm] = useState("");

  const [selectedDoctor, setSelectedDoctor] = useState<
    VerifiedDoctor | RejectedDoctor | null
  >(null);
  ...
  const handleConfirmApprove = async (doctorId: number, note: string) => {
    ...
    try {
      await dispatch(
        fetchApprovePendingDoctor({ id: String(doctorId), note }),
      ).unwrap();
  ...
  const handleConfirmReject = async (doctorId: number, reason: string) => {
    ...
    try {
      await dispatch(
        fetchRejectPendingDoctor({ id: String(doctorId), reason }),
      ).unwrap();
  ...
  const activeList =
    activeTab === "verified" ? filteredVerified : filteredRejected;
    ...
    <DashboardLayout pageTitle="Doctors Management">
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 justify-between">
          ...
          >
            {(["verified", "rejected"] as const).map((tab) => {
              ...
              const activeColor =
                tab === "verified" ? "var(--color-success)" : "#dc2626";
              return (
                <button
                  key={tab}
                  onClick={() => {
        ...
        rejectedData.data.length === 0 ? (
          ...
          >
            <User
              className="w-10 h-10 mx-auto mb-3.5"
              style={{ color: "var(--color-border)" }}
            />
            <h4
      ...
      {showApproveModal && targetDoctor && (
        <ApproveDoctorModal
          ...
          doctor={targetDoctor}
          submitting={
            actionDoctorId === targetDoctor.doctorId &&
            approveActionState.loading
          }
        />
      )}
